...
async def callbacks_faq(callback: CallbackQuery):
    action = callback.data.split("_")[1]
    await callback.message.answer(FAQ[int(action)][1], reply_markup=faq_answer_keyboard())
...
async def callbacks_plus(callback: CallbackQuery):
    ...
    if action[1] == 'plus':
        keyboard, product, count = await product_keyboard(prod_id=prod_id, count=count + 1)
        await callback.message.answer_photo(photo=FSInputFile(
            path=f'aiogramBariev/{product.image}',
...
async def callbacks_plus(callback: CallbackQuery):
    ...
    await set_or_create_basket(tg_id=tg_id, product_id=prod_id, quantity=count)
    await callback.message.answer('Product added to cart', reply_markup=main_keyboard())
...
async def callbacks_del_product(callback: CallbackQuery, ):
    ...
    await del_product(tg_id=callback.from_user.id, product_id=int(action))
